...
const Formulario = ( {pacientes,setPacientes, error, setError, paciente, setPaciente }) => {
    ...
    const handleSubmit = (e) => {
      ...
      if([nombre, propietario, email, fechaAlta, sintomas].includes('')) {
        console.table('Hay al menos un campo vacio');
        setError(true);
        return;
      } 
  ...
  return (
    <div className="md:w-1/2 lg:w-2/5 ml-3 mr-3">
      ...
      <p className="text-lg mt-4 text-center mb-10">
        Añade Pacientes y {' '} <span className="text-indigo-600 font-bold">
           Administralos
        </span>
      </p>
